...
import { Database, Search, LogIn } from 'lucide-react';
...
const Landing: React.FC = () => {
  ...
  const handleBatchQuery = () => {
    // Consulta em lote está desativada
  };
    ...
    <div className="min-h-screen bg-neutral-50">
      <header className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          {!isAuthenticated ? (
            ...
            <span className="text-neutral-600">Bem-vindo, admin</span>
          )}
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-12 md:py-20">
        ...
        >
          ...
          >
            A plataforma moderna para consultas e gerenciamento eficiente de dados. Escolha seu tipo de consulta abaixo para começar.
          </motion.p>
...
const QueryOption: React.FC<QueryOptionProps> = ({ icon, title, description, onClick, disabled = false }) => {
  return (
    <motion.div
      whileHover={{ y: disabled ? 0 : -5, boxShadow: disabled ? 'none' : '0 8px 30px rgba(0, 122, 255, 0.2)' }}
    ...
    >
      <div className="flex justify-center mb-4">
        <div className={`w-16 h-16 rounded-full ${disabled ? 'bg-neutral-300' : 'europa-gradient'} flex items-center justify-center`}>
          {icon}
        </div>
      </div>
